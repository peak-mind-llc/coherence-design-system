...
- **Data-vis colors and `canvas-tokens.ts` are off-limits.** The clinical visualizations have their own constrained palette ([Coherence_Workstation_Spec.md](../../Coherence_Workstation_Spec.md)). Don't reach for design-system tokens inside data-vis renderers. **Two sanctioned exceptions**, both clinical ramps minted into the kit so a clinical surface gets DS-native, gated colours instead of raw hex. These are the only two; other clinical-viz colour stays in the spec palette, and a *third* ramp needs a fresh ruling.
  - **Vigilance severity ramp** (`foreground-vigilance-{0,a1,a2,a3,b1,b2,b3,c}`) — an 8-step Loomis alertness→sleep scale, for the redesigned Vigilance strip (`VigilanceStripV2`). (Red end capped at 500/600/700 for legibility on the dark muted track.)
  - **Annotation-label ramp** (`{foreground,background}-label-{peak-primary,peak-compensatory,peak-secondary,unclear,suppressed,expected-low}`) — the peak/valley label vocabulary a clinician assigns when marking a spectral fingerprint (ruled July 2026). The `foreground-` role is the label's ink and the `background-` role its fill: the annotation popover's selected chip fills with its label colour, and the **same** `foreground-` token paints that peak's marker on the spectrum/IC canvases. That single-source property is the whole point of the ruling — before it, the chip and the marker were two hex literals that could drift. Consumed via `PEAK_LABELS`/`VALLEY_LABELS` in `@coherence/clinical-sdk`, which carry `var(--foreground-label-*)` references rather than hex.
  - Both ramps are **theme-independent** (same primitive in light + dark), so a stage/label reads the same colour in either theme.
  - **Canvas consumers must resolve the token.** `ctx.fillStyle = 'var(--foreground-label-rigid)'` is silently ignored by the 2D context — the assignment fails and the *previous* colour keeps painting. Resolve against the chart's **host element** (not `:root`, which returns light values inside a dark subtree); `UPlotMiniSpectrum`'s `resolveColor` is the reference implementation. SVG consumers can use the `var(…)` string directly.
